...
def QNetwork_exam():
    ...
    state = torch.rand(env.observation_space.shape[0])
    print(f"state:{state}")
...
class DqnAgent():
    ...
    def update_q(self):
        ...
        maxq = torch.max(
            self.target_qnet(torch.tensor(batch["next_states"], dtype=torch.float)),
            dim=1,
        ).values
...
for episode in range(num_episode):
    ...
    episode_reward = 0
    for t in range(max_steps):
        action = agent.get_action(state, episode=episode)  # 行動を選択
        next_state, reward, done, terminated, _ = env.step(action)
        episode_reward += reward
        transition = {
            "state": state,
